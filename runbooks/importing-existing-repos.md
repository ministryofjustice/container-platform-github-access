# Runbook: Adding or Importing Repositories
...
> Missing a setting could break a live site, remove team access,
> or destroy rulesets.
...
Add the repository to `github-repositories.tf` with settings that match what the API returned.
...
- **`is_template`**: if it's a template repository, set this to `true`
...
> Do not set `use_template = true` for repos that already exist.
...
# Only import teams that exist on the repo AND are in your config
import {
